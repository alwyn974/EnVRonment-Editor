...
func _update_transform_gizmo_view() -> void:
	...
	var gizmoD := float(max(abs(p.distance_to(xform.origin)), 1e-06))
	...
	if mode == ToolMode.ALL || mode == ToolMode.MOVE:
		...
		if col_axis == -1:
			...
			for i in range(3):
				...
				var r : Variant = plane.intersects_ray(ray_pos, ray)
...
func _update_transform(shift: bool) -> void:
	...
	match _edit.mode:
		TransformMode.SCALE:
			...
			var sintersection : Variant = splane.intersects_ray(ray_pos, ray)
			if sintersection == null:
				return
			
			var sclick : Variant = splane.intersects_ray(_edit.click_ray_pos, _edit.click_ray)
			if sclick == null:
				return
		...
		TransformMode.TRANSLATE:
			...
			var tintersection : Variant = tplane.intersects_ray(ray_pos, ray)
			if tintersection == null:
				return
			
			var tclick : Variant = tplane.intersects_ray(_edit.click_ray_pos, _edit.click_ray)
			if tclick == null:
				return
		...
		TransformMode.ROTATE:
			...
			var rintersection : Variant = rplane.intersects_ray(ray_pos, ray)
			if rintersection == null:
				return
			
			var rclick : Variant = rplane.intersects_ray(_edit.click_ray_pos, _edit.click_ray)
			if rclick == null:
				return
...
func _calculate_spatial_bounds(parent: Node3D, omit_top_level: bool = false, bounds_orientation: Transform3D = Transform3D.IDENTITY) -> AABB:
	...
	print(parent.global_transform)
	var t := parent.global_transform
	var ta := parent.transform
	...
	if parent is VisualInstance3D:
		...
		bounds = AABB()
	bounds = xform(xform_to_top_level_parent_space, bounds)
	...
	for child in parent.get_children():
		...
		if !(omit_top_level and child.top_level):
			...
			print(child.name, " ", child_bounds)
			bounds = bounds.merge(child_bounds)
	
	return bounds

func xform(xfom: Transform3D, bounds: AABB) -> AABB:
	...
	for i in range(3):
		tmin[i] = xfom.origin[i]
		tmax[i] = xfom.origin[i]
		for j in range(3):
			var e = xfom.basis[j][i] * min[j]
			var f = xfom.basis[j][i] * max[j]
			if e < f:
				tmin[i] += e
				tmax[i] += f
			else:
